...
async function getWeather(cityAdcode) {
    try {
        const response = await axios.get(`${WEATHER_API_URL}?key=${API_KEY}&city=${cityAdcode}&extensions=base&output=JSON`);
...
// 定义MCP工具
mcpServer.tool("getWeather", "获取某个城市的天气信息", {
...
async function main() {
    ...
    mcpServer.server.sendLoggingMessage({ level: 'info', data: 'Server started successfully', });
}
main().catch((error) => {
