...
let cityMappings: CityMapping[] = [];

// 读取 JSON 数据并存入内存
function loadCityMappings(): void {
  ...
  try {
    ...
    // 使用类型断言确保数据结构一致性
    cityMappings = (jsonData as Array<{name: string; adcode: string}>).map(item => ({
      name: item.name.trim(),
...
// 模糊搜索城市编码
function findAdcode(cityName: string): string {
  const candidates = cityMappings.filter(item =>
      item.name.startsWith(cityName)
  );
...
export async function getCityAdcode(cityName: string): Promise<string> {
